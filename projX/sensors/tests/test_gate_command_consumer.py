...
class GateCommandOpenGateTests(unittest.TestCase):
    def test_open_gate_approved_payment_opens_exit_gate(self):
        ...
        response, park_id, gate_event = sim.on_gate_command(_command("OPEN_GATE", "park-1", direction="exit"))
    ...
    def test_block_gate_rejected_payment_blocks_exit_gate(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, park_id, gate_event = sim.on_gate_command(_command("BLOCK_GATE", "park-1", direction="exit"))
    ...
    def test_open_gate_denied_when_gate_in_fault(self):
        ...
        response, _, _ = sim.on_gate_command(_command("OPEN_GATE", "park-1", direction="exit"))
    ...
    def test_block_gate_denied_when_gate_in_fault(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        sim._gates["park-1"]["exit"].state = GateState.FAULT
        response, _, _ = sim.on_gate_command(_command("BLOCK_GATE", "park-1", direction="exit"))

        self.assertEqual(response["status"], "DENIED")

    def test_unknown_park_returns_denied(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, _, _ = sim.on_gate_command(_command("OPEN_GATE", "park-999", direction="exit"))
    ...
    def test_unknown_command_type_returns_denied(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, _, _ = sim.on_gate_command(_command("TELEPORT_GATE", "park-1", direction="exit"))
    ...
    def test_plate_forwarded_to_gate_event(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        _, _, gate_event = sim.on_gate_command(_command("OPEN_GATE", "park-1", plate="ZZ-99-ZZ", direction="exit"))
