...
class ParkGateExecuteCommandTests(unittest.TestCase):
    ...
    def test_open_command_executes_from_closed(self):
        gate = self._gate()
        status, reason, event = gate.execute_command("cmd-1", GateCommandType.OPEN_GATE, "park-1", "Park 1")
    ...
    def test_close_command_executes_from_open(self):
        gate = self._gate()
        gate.state = GateState.OPEN
        status, reason, event = gate.execute_command("cmd-2", GateCommandType.CLOSE_GATE, "park-1", "Park 1")
        self.assertEqual(status, GateCommandStatus.EXECUTED)
...
class GateSimulatorOnCommandTests(unittest.TestCase):
    def test_open_command_executes_updates_state(self):
        ...
        response, key, gate_event = sim.on_gate_command(_command("cmd-1", "park-1", "entry", "OPEN_GATE"))
    ...
    def test_close_command_executes_from_open(self):
        ...
        response, key, gate_event = sim.on_gate_command(_command("cmd-2", "park-1", "entry", "CLOSE_GATE"))
        self.assertEqual(response["status"], "EXECUTED")
    ...
    def test_command_denied_when_gate_in_fault(self):
        ...
        response, key, gate_event = sim.on_gate_command(_command("cmd-3", "park-1", "exit", "OPEN_GATE"))
    ...
    def test_command_denied_for_unknown_park(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, key, gate_event = sim.on_gate_command(_command("cmd-4", "park-999", "entry", "OPEN_GATE"))
        self.assertEqual(response["status"], "DENIED")
    ...
    def test_command_denied_for_unknown_command_type(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, key, gate_event = sim.on_gate_command(_command("cmd-5", "park-1", "entry", "LAUNCH_ROCKETS"))
        self.assertEqual(response["status"], "DENIED")
        self.assertEqual(response["reason"], "unknown_command_type")

    def test_command_id_correlates_in_response(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, key, _ = sim.on_gate_command(_command("my-unique-cmd-id", "park-1", "entry", "OPEN_GATE"))
    ...
    def test_response_key_matches_park_id(self):
        ...
        response, key, _ = sim.on_gate_command(_command("cmd-1", "park-2", "entry", "OPEN_GATE"))
        self.assertEqual(key, "park-2")

    def test_exit_gate_open_command(self):
        sim = GateSimulator(parks=_parks(1), seed=1)
        response, key, gate_event = sim.on_gate_command(_command("cmd-6", "park-1", "exit", "OPEN_GATE"))
        self.assertEqual(response["status"], "EXECUTED")
...
class GateCommandKafkaFlowTests(unittest.TestCase):
    """Integration-style: simulate full command → response Kafka flow with stub publishers."""
    ...
    def test_executed_command_publishes_response_and_gate_event(self):
        ...
        response_msg = next(p[2] for p in published if p[0] == KAFKA_TOPIC_GATE_RESPONSES)
    ...
    def test_commands_and_events_use_separate_topics(self):
        from config import KAFKA_TOPIC_GATE, KAFKA_TOPIC_GATE_COMMANDS, KAFKA_TOPIC_GATE_RESPONSES
