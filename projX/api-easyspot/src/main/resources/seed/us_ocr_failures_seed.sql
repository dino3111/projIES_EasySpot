...
WITH
park AS (
    SELECT id AS park_id FROM parking_lots ORDER BY id LIMIT 1
),
spot AS (
    SELECT id AS spot_id FROM parking_spots
    WHERE parking_lot_id = (SELECT park_id FROM park)
    ORDER BY spot_row, spot_col
    LIMIT 1
)
...
SELECT
    ...
    NOW() - (scenario.minutes_ago * interval '1 minute'),
    jsonb_build_object('source', 'seed', 'scenario', scenario.label)
FROM park AS p, spot AS s,
(VALUES
...
) AS scenario (label, plate, confidence, direction, failure_mode, minutes_ago)
