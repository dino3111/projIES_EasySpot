...
const PROFILE_OPTIONS: {
  ...
  { id: 'MANAGER',   icon: 'fa-building',           label: 'Gestor de Parques',     desc: 'Dashboard, tarifas e ocorrências', color: '#f59e0b', home: '/manager/dashboard' },
  { id: 'TECHNICAL', icon: 'fa-screwdriver-wrench', label: 'Técnico de Manutenção', desc: 'Diagnóstico, sensores e tarefas',  color: '#3b82f6', home: '/technician/dashboard' },
];
      ...
      <div className="flex items-center justify-between px-4 h-14 max-w-screen-2xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          {profile !== 'DRIVER' && (
            ...
            >
              <i className={`fas ${currentOpt.icon}`} style={{ fontSize: '0.6rem' }} aria-hidden="true" />
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              aria-haspopup="true"
            >
              <i className={`fas ${currentOpt.icon} text-white text-lg`} aria-hidden="true" />
              <span
...
interface ProfileDropdownProps {
  profile: AppProfile;
  currentOpt: (typeof PROFILE_OPTIONS)[0];
  options: typeof PROFILE_OPTIONS;
  onSwitch: (id: AppProfile, home: string) => void;
  onClose: () => void;
}

function ProfileDropdown({ profile, currentOpt, options, onSwitch, onClose }: ProfileDropdownProps) {
  return (
    <div
    ...
    >
      ...
      {options.map((opt, idx) => (
        <div key={opt.id}>
          <button
            role="menuitem"
            onClick={() => onSwitch(opt.id, opt.home)}
          ...
          >
            <div
              ...
              aria-hidden="true"
            >
              <i className={`fas ${opt.icon}`} style={{ fontSize: '0.9rem' }} />
            </div>
            ...
            {profile === opt.id && (
              <i className="fas fa-circle-check text-primary flex-shrink-0" style={{ fontSize: '0.85rem' }} aria-hidden="true" />
            )}
          </button>
          {idx < options.length - 1 && <div className="h-px bg-border mx-3" aria-hidden="true" />}
        </div>
      ))}
      ...
      >
        <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-muted text-muted-foreground" aria-hidden="true">
          <i className="fas fa-gear" style={{ fontSize: '0.9rem' }} />
        </div>
        <div className="flex-1 min-w-0">
