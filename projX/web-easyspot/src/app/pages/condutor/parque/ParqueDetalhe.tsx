...
import { InfoBox, amenityIcon, amenityLabel } from './parqueShared';
import { TabGeral } from './TabGeral';
import { TabMapa } from './TabMapa';
import { TabEV } from './TabEV';
import { TabAcessibilidade } from './TabAcessibilidade';
...
export function ParqueDetalhe() {
  ...
  const initialTab = useMemo<Tab>(() => {
    ...
    if (myVehicle?.isAccessible && totalAcc > 0) return 'acessibilidade';
    return 'geral';
  }, [lot?.id, myVehicle?.id]);
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm flex flex-col flex-1 pb-4">
            ...
            <div className="p-4 sm:p-5 flex-1">
              {activeTab === 'geral' && (
                <TabGeral lot={lot} occupied={occupied} occupancyPct={occupancyPct} isFull={isFull} isAlmostFull={isAlmostFull} statusHex={statusHex} />
              )}
              {activeTab === 'mapa' && (
                <TabMapa lot={lot} activeFloorIdx={activeFloorIdx} setActiveFloorIdx={setActiveFloorIdx} />
              )}
              {activeTab === 'ev' && <TabEV lot={lot} myVehicle={myVehicle} />}
              {activeTab === 'acessibilidade' && <TabAcessibilidade lot={lot} />}
              {activeTab === 'tarifas' && <TabTarifas lot={lot} />}
            </div>
          </div>
        </div>
      </div>
